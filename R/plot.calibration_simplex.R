...
plot.calibration_simplex = function(x,
  ...
  triangle= function(centers = F) {#Höhe der Hexagone ist 1
    a = n - 1 #a: Grundseite Dreieck, s: Seite Hexagon
    s = 1/sqrt(3)  #~1/Wurzel(3) berechnet, sodass H?he = 1...
    sin60 = sqrt(3)/2
    x = c(0,sin60*a,0)
    y = c(0,a/2,a)
  ...
  plot(H,border="darkgrey",add = T) #col changed to border in spatstat package...
